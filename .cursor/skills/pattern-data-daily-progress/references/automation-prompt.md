...
1. Bootstrap Daily Progress/pattern-data-delivery-progress-YYYY-MM-DD.md from the most recent file in Daily Progress/ if today's file does not exist. Update only the As of header, Jira sync date, and standup/Austin input dates. Leave historical event dates unchanged. Use default delivery targets from domain-decisions.md (UAT sandbox **2026-09-01** · Production **2026-09-08**) in the header, Status at a glance, and Path to UAT & Production unless Austin's last plan or a slip says otherwise.
2. If today's ChartSwap standup `.docx`, `PDReviewWithAustin-YYYY-MM-DD.docx`, or Austin meeting `.docx` is missing from Transcript/, skip transcript extraction. Keep Austin's last deployment plan. Default Islam's Team focus to PD sandbox testing of LNI-3763 RequestShare unless a transcript says otherwise.
