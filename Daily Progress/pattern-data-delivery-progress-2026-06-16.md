...
**Go-live goal:** End of June 2026 · **Epic:** [OC-9223](https://ontellus.atlassian.net/browse/OC-9223) · **As of:** 2026-06-16
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **Pre-UAT** | Youssef | In progress | <li>4/7 providers tested [STD, PFP, RR, Waystar]</li><li>RR payment bugs cleared and retested (OC-9529, OC-9531 closed; full flow retest 2026-06-08)</li><li>Hamed completed ShareCare and Updox sandbox passes (2026-06-10); ShareCare report conflict logged; Updox deprioritized per Austin</li><li>Youssef testing pass largely complete (2026-06-15); RR card-payment scenarios partially verified; demo with Michael pending on remaining RR/CNR items</li><li>Islam pushed Wave 1 unit tests to feature branch (2026-06-15) — ~10 new test classes for Payment Management, Invoice, Prefill; coverage brought to ≥75% on under-covered classes; deployed to sandbox</li><li>Michael completed most Austin transaction + Invoice/S3 comments (2026-06-15); validation working on transaction object</li><li>Youssef retested open bugs (2026-06-16) — current fix set working; remainder of scenarios tested OK; **stable on current bug batch**</li><li>Hamed ShareCare partial retest (2026-06-16) — report scenario OK; provider cost not visible in VF pending Michael filter fix</li> | <ul><li>8 open payment bugs in QA / pending review on Jira — RR, PFP, ShareCare Major items still open until QA closes</li><li>Michael address merge/rebase class gaps on master (Austin temp workaround — follow up in PD standup)</li><li>Youssef send deferred comment list to Michael; new chat findings need Jira tickets — access still blocked</li><li>Michael finalize remaining Austin field-removal items after build validation</li><li>Sarah start CIOX data classes on sandbox (not started; on leave Thu; Jira access blocked)</li><li>Islam + Michael code review — deferred to next week (Islam on leave 6/16–6/17)</li><li>Wave 1 unit-test sign-off before UAT — Islam + Michael</li><li>Salah confirm Wave 1 / Pre-UAT dates with Austin today — likely slip past 6/17</li></ul> | 2026-05-21 | **2026-06-17** *(at risk — dates TBD pending Austin)* | <li>Sandbox goal: **pre-fulfillment parity with UAT** — not chasing fulfillment simulation on sandbox. CIOX fulfillment gap accepted per 06-05 agreement; escalate only if new non-fulfillment issues surface.</li><li>External-provider fulfillment simulation deferred to UAT (Eric).</li><li>Automation paused — manual retest for now.</li><li>**06/14 Pre-UAT forecast missed** — Wave 1 UAT did not start 6/15; Austin did not follow up on deployment.</li><li>**Updox deprioritized** per Austin (likely not active on production; Hamed already tested replacement path).</li><li>Team adopting feature-branch + PR workflow; Islam's unit tests on sandbox branch.</li><li>Post-Atlassian migration: Jira access still unresolved — Jessica unable to grant; Youssef cannot board new items.</li><li>**06/16 standup:** Youssef declared **stable version** on current fixes; deferred Austin items held for demo; Islam out today and tomorrow.</li> |
...
| **UAT** | Van | Not started | — | <ul><li>Wave 1 (6/15): Prefill API, CC Management, Invoice Upload</li><li>Wave 2 (6/17): Status Retrieval API, Status Sync Job</li><li>Wave 3 (6/22): Auto-Pay via CC (TPR)</li></ul> | 2026-06-15 – 2026-06-25 | **At risk** — Wave 1 UAT missed 6/15; Austin demo postponed to 6/22; Salah confirming slip with Austin today | Wave 2 dates may be delayed as it depends on PD webhook details which is not provided yet (see Risks). Austin also waiting on ShareCare partnership item before Wave 1 push. |
...
| --- | --- | --- | --- | --- | --- | --- |
| Wave 1 | Prefill Order form API | API to prefill order form | Yes | 2026-06-15 | 2026-06-17 | Pre-UAT not closed 6/14 — UAT start missed 6/15; dates TBD |
...
| Wave 1 | Invoice Upload to S3 | On successful Request payment: upload with Type **Invoice**. Prod path: `Chartswap/Invoices`. | Yes | 2026-06-15 | 2026-06-17 | Validate VF bucket filtering — invoice filter may hide provider cost on portal; Michael fixing 2026-06-16. |
...
*Process (2026-06-16 standup):* When handing a bug to QA, dev should demo the fix with the tester first to reduce ping-pong.

*Synced from Jira epic [OC-9223](https://ontellus.atlassian.net/browse/OC-9223) on 2026-06-16 (live).*

**Eight open payment bugs** on the epic — three **Major** (ShareCare, RR, PFP) block provider sign-off or PFP testing; five **Minor** (Payment Management + AutoPay draft path) still block or slow Pre-UAT scenario testing. Jira statuses unchanged; standup retest progress noted below.

### ShareCare

| Bug | What's wrong | Jira status | Assignee | Notes |
| --- | --- | --- | --- | --- |
| [OC-9562](https://ontellus.atlassian.net/browse/OC-9562) | Authorization submit fails with Too many SOQL queries (101) on `enterrequestdatasc` — blocks ShareCare workflow completion | **QA** | Mohamed Hamed | **New 2026-06-09** — Major — no workaround. **Update 2026-06-10:** Michael also fixing sandbox company routing — ShareCare S3 uploads were hitting production bucket. **Update 2026-06-15:** Michael fixing cost/full-film scenario in sandbox. **Update 2026-06-16:** Hamed retested report scenario — OK; provider cost not visible in VF (provider dot missing). Michael adjusting invoice/cost filter today |

### Record Retrieval

| Bug | What's wrong | Jira status | Assignee | Notes |
| --- | --- | --- | --- | --- |
| [OC-9566](https://ontellus.atlassian.net/browse/OC-9566) | Request reaches **Completed** after prepayment with no records uploaded and no recovery path (REQ-23792287) — completion gated on payment instead of fulfillment | **QA** | Youssef Yahia | **New 2026-06-10** — Major — no workaround. **Update 2026-06-15:** Youssef finished testing pass; demo with Michael pending. **Update 2026-06-16:** Youssef retested — both scenarios working; provider-portal cost field hidden by new invoice filter — Michael fixing filter today |
...
| [OC-9389](https://ontellus.atlassian.net/browse/OC-9389) | After Replace/Remove card, payment reinitiation for affected requests not verifiable in UI (DEC-050) | **Pending Code Review** | Sara Hassan | Minor — verifiability gap. **Update 2026-06-15:** Sarah discussed fix with Michael but has not pushed demo branch; Heba retest still pending |

### AutoPay Submission Flow

| Bug | What's wrong | Jira status | Assignee | Notes |
| --- | --- | --- | --- | --- |
| [OC-9576](https://ontellus.atlassian.net/browse/OC-9576) | PFP **1st payment never initiated** — request stuck in Pending Payment with no failure note and no Retry / Change Card / Accept actions (REQ-23792610) | **QA** | Youssef Yahia | **New 2026-06-11** — Major — no workaround; blocks PFP payment + fulfillment testing. **Update 2026-06-16:** Youssef retested remainder of scenarios — OK on current fix set |
...
| Flow | Assignee | Date |
| --- | --- | --- |
| Record Retrieval (post OC-9566 fix) | Youssef | 2026-06-16 — retest OK; pending Michael filter fix for provider cost display |
| ShareCare (post Michael cost/full-film + VF visibility fix) | Hamed | 2026-06-16 — partial; blocked on provider dot / cost in VF |
...
| Member | Focus |
| --- | --- |
| Michael | Provider-portal invoice/cost filter fix; ShareCare VF visibility for Hamed; merge/rebase class gaps (Austin workaround); Youssef comment list; OC-9615 prefill logout in QA |
| Islam | **On leave 6/16–6/17** — unit-test code review with Michael deferred to next week |
| Sarah | **On leave Thursday** — CIOX data classes not started; OC-9389 demo branch pending; Jira access blocked |
| Hamed | ShareCare retest in progress — blocked on Michael VF provider visibility fix |
| Youssef | Stable on current bug batch; send comments to Michael; new chat findings need Jira tickets (access blocked) |
| Salah | Confirm Wave 1 UAT slip and revised dates with Austin today; escalate Jira access |
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| 1   | **Pre-UAT missed 06/14 forecast; Wave 1 UAT missed 6/15.** Eight payment bugs remain open in Jira QA, Wave 1 UAT did not start as planned, and Austin has not confirmed revised dates — June go-live at risk despite Youssef declaring stable on the current fix batch (2026-06-16). Michael's Austin transaction + Invoice/S3 comments are largely done, freeing some capacity.                                                                                          | <ul><li>Salah confirm slip duration with Austin today</li><li>Re-baseline Pre-UAT forecast once Austin responds</li><li>Michael demo fixes with testers before QA handoff</li><li>Escalate if go-live is threatened</li></ul> | High     |
...
| 6   | **Code updates can reopen fixed bugs and break previously passing scenarios.** Ongoing fixes and Austin-driven changes may regress areas already signed off in Pre-UAT, forcing duplicate retest cycles and eroding confidence in the 06/14 close-out. Merge/rebase class gaps on master flagged 2026-06-16.                                                                                                                      | <ul><li>Include detailed testing steps, expected results, and related areas to retest after updates in each story</li><li>Add screenshots of the resolution as evidence in a Jira comment when closing or handing off a fix</li><li>Adopt feature-branch + PR workflow to preserve change history</li><li>Michael follow up with Austin on master merge gaps</li></ul>                                                                                                 | High     |
| 7   | **Post-Atlassian migration Jira access gaps.** Sarah, Youssef, and Hamed still lack project/board access — Jessica unable to grant (2026-06-16). Youssef cannot board new chat findings; slows QA handoffs and board visibility.                                                                                                                      | <ul><li>Salah escalate with Austin/IT today</li><li>Use BO board and Teams for interim coordination</li><li>Each person document their specific access issue on Austin tracking sheet</li></ul>                                                                                                 | Medium   |
