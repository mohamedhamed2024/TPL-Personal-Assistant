...
**Go-live goal:** End of June 2026 · **Epic:** [OC-9223](https://ontellus.atlassian.net/browse/OC-9223) · **As of:** 2026-06-15
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **Pre-UAT** | Youssef | In progress | <li>4/7 providers tested [STD, PFP, RR, Waystar]</li><li>RR payment bugs cleared and retested (OC-9529, OC-9531 closed; full flow retest 2026-06-08)</li><li>CIOX payment bugs closed (OC-9532, OC-9552 closed 2026-06-08)</li><li>Hamed completed ShareCare and Updox sandbox passes (2026-06-10); ShareCare report conflict logged; Updox deprioritized per Austin</li><li>Youssef testing pass largely complete (2026-06-15); RR card-payment scenarios partially verified; demo with Michael pending on remaining RR/CNR items</li><li>Islam pushed Wave 1 unit tests to feature branch (2026-06-15) — ~10 new test classes for Payment Management, Invoice, Prefill; coverage brought to ≥75% on under-covered classes</li><li>Michael completed most Austin transaction + Invoice/S3 comments (2026-06-15); validation working on transaction object</li> | <ul><li>8 open payment bugs in QA / pending review — RR, PFP, ShareCare Major blockers</li><li>Michael fix ShareCare sandbox cost/full-film scenario — retry with Hamed tomorrow (2026-06-16)</li><li>Michael finalize remaining Austin field-removal items after build validation</li><li>Sarah start CIOX data classes on sandbox (not started; blocked on Jira access + bug fixes)</li><li>Youssef + Michael demo on remaining fixes before deploy</li><li>Islam confirm unused test classes with Michael; code review with Michael</li><li>Wave 1 unit-test sign-off before UAT — Islam + Michael</li><li>Islam verify CNR file display on UAT after reject flow</li></ul> | 2026-05-21 | **2026-06-17** | <li>Sandbox goal: **pre-fulfillment parity with UAT** — not chasing fulfillment simulation on sandbox. CIOX fulfillment gap accepted per 06-05 agreement; escalate only if new non-fulfillment issues surface.</li><li>External-provider fulfillment simulation deferred to UAT (Eric).</li><li>Automation paused — manual retest for now.</li><li>**06/14 Pre-UAT forecast missed** — bugs still in QA; ShareCare blocked on Michael's cost/full-film sandbox fix.</li><li>**Updox deprioritized** per Austin (likely not active on production; Hamed already tested replacement path).</li><li>Team adopting feature-branch + PR workflow (discussed 2026-06-15); data-script exists for local sandbox setup.</li><li>Post-Atlassian migration: several team members lost Jira project access (Sarah, Youssef, Hamed) — Jessica notified; may slow QA handoffs.</li> |
...
| **UAT** | Van | Not started | — | <ul><li>Wave 1 (6/15): Prefill API, CC Management, Invoice Upload</li><li>Wave 2 (6/17): Status Retrieval API, Status Sync Job</li><li>Wave 3 (6/22): Auto-Pay via CC (TPR)</li></ul> | 2026-06-15 – 2026-06-25 | **At risk** — Wave 1 UAT target 06/15 unlikely; Pre-UAT did not close 06/14 | Wave 2 dates may be delayed as it depends on PD webhook details which is not provided yet (see Risks). |
...
| --- | --- | --- | --- | --- | --- | --- |
| Wave 1 | Prefill Order form API | API to prefill order form | Yes | 2026-06-15 | 2026-06-17 | Pre-UAT not closed 06/14 — UAT start at risk |
...
*Process (2026-06-15 standup):* When handing a bug to QA, dev should demo the fix with the tester first to reduce ping-pong.
...
| --- | --- | --- | --- | --- |
| [OC-9562](https://ontellus.atlassian.net/browse/OC-9562) | Authorization submit fails with Too many SOQL queries (101) on `enterrequestdatasc` — blocks ShareCare workflow completion | **QA** | Mohamed Hamed | **New 2026-06-09** — Major — no workaround. **Update 2026-06-10:** Michael also fixing sandbox company routing — ShareCare S3 uploads were hitting production bucket. **Update 2026-06-15:** Michael fixing cost/full-film scenario in sandbox today; Hamed to retry tomorrow after fix lands |

### Record Retrieval

| Bug | What's wrong | Jira status | Assignee | Notes |
| --- | --- | --- | --- | --- |
| [OC-9566](https://ontellus.atlassian.net/browse/OC-9566) | Request reaches **Completed** after prepayment with no records uploaded and no recovery path (REQ-23792287) — completion gated on payment instead of fulfillment | **QA** | Youssef Yahia | **New 2026-06-10** — Youssef retest; Hamed's prior RR flow paid-then-fulfilled correctly. Major — no workaround. **Update 2026-06-15:** Youssef finished testing pass; demo with Michael pending on remaining RR/CNR issues |
...
| Flow | Assignee | Date |
| --- | --- | --- |
| Record Retrieval (post OC-9566 fix) | Youssef | TBD — demo with Michael 2026-06-15 |
| ShareCare (post Michael cost/full-film fix) | Hamed | 2026-06-16 |
...
| Member | Focus |
| --- | --- |
| Michael | ShareCare sandbox cost/full-film fix (today); demo with Youssef on remaining RR/CNR items; code review unit tests with Islam; remaining Austin field-removal items after validation |
| Islam | Confirm unused test classes with Michael; Wave 1 unit-test coverage on feature branch — code review with Michael |
| Sarah | Start CIOX data classes on sandbox (after bugs clear); OC-9389 demo branch pending; Jira access blocked post-migration |
| Hamed | ShareCare retry blocked on Michael's sandbox fix — scheduled 2026-06-16 |
| Youssef | Demo with Michael on remaining fixes; RR + Payment Management scenario testing blocked by open QA bugs |
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| 1   | **Pre-UAT missed 06/14 forecast; Wave 1 UAT at risk.** Eight payment bugs remain in QA, ShareCare still blocked, and Michael is fixing sandbox scenarios — Wave 1 UAT was targeted for 06/15 but Pre-UAT has not closed, threatening the June go-live. Michael's Austin transaction + Invoice/S3 comments are largely done (2026-06-15), freeing some capacity.                                                                                          | <ul><li>Re-baseline Pre-UAT to <strong>2026-06-17</strong> and escalate Wave 1 slip to Van/Austin</li><li>Michael demo fixes with testers before QA handoff</li><li>Track all changes against the [client release plan](#client-release-plan-received-2026-06-01)</li><li>Escalate if go-live is threatened</li></ul> | High     |
...
| 6   | **Code updates can reopen fixed bugs and break previously passing scenarios.** Ongoing fixes and Austin-driven changes may regress areas already signed off in Pre-UAT, forcing duplicate retest cycles and eroding confidence in the 06/14 close-out.                                                                                                                      | <ul><li>Include detailed testing steps, expected results, and related areas to retest after updates in each story</li><li>Add screenshots of the resolution as evidence in a Jira comment when closing or handing off a fix</li><li>Adopt feature-branch + PR workflow to preserve change history (discussed 2026-06-15)</li></ul>                                                                                                 | High   |
| 7   | **Post-Atlassian migration Jira access gaps.** Sarah, Youssef, and Hamed lost project/board access after the migration — permission requests unanswered (Sunday holiday). Slows bug triage, QA handoffs, and board visibility until restored.                                                                                                                      | <ul><li>Jessica notified (2026-06-15); follow up with IT/admin</li><li>Use BO board and Teams for interim coordination</li></ul>                                                                                                 | Medium   |
